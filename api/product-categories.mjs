...
const brownFeedProductCategories = pgTable('brown_feed_product_categories', {
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const categories = await db.select().from(brownFeedProductCategories).orderBy(brownFeedProductCategories.displayOrder);
      
      if (categories.length === 0) {
        // Initialize with Brown Feed Store categories
        ...
        await db.insert(brownFeedProductCategories).values(defaultCategories);
    ...
    if (req.method === 'POST') {
      const [newCategory] = await db.insert(brownFeedProductCategories).values(req.body).returning();
    ...
    if (req.method === 'PUT') {
      ...
      const [updated] = await db
        .update(brownFeedProductCategories)
        .set(req.body)
        .where(eq(brownFeedProductCategories.id, parseInt(id)))
        .returning();
