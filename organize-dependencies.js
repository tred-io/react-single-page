...
async function updateGitHubPackageJson() {
  ...
  try {
    ...
    // Build-time dependencies that need to be in dependencies for Vercel
    const buildTimeDeps = [
      'autoprefixer',
      'postcss', 
      'tailwindcss',
    ...
    const updateData = {
      message: 'Fix: Move build-time dependencies to dependencies for Vercel',
      content: Buffer.from(fixedContent).toString('base64'),
      sha: currentFile.sha
    };

    const result = await makeRequest(updateOptions, JSON.stringify(updateData));
    console.log('Successfully organized package dependencies for Vercel build');
