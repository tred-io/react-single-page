...
async function updateGitHubPackageJson() {
  ...
  try {
    ...
    // Remove autoprefixer from devDependencies since it's already in dependencies
    if (packageObj.devDependencies && packageObj.devDependencies.autoprefixer) {
      delete packageObj.devDependencies.autoprefixer;
    }
    ...
    const updateData = {
      message: 'Fix: Remove autoprefixer duplicate from devDependencies',
      content: Buffer.from(fixedContent).toString('base64'),
      sha: currentFile.sha
    };

    const result = await makeRequest(updateOptions, JSON.stringify(updateData));
    console.log('Successfully removed autoprefixer duplicate from devDependencies');
